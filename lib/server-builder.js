...
const DEPENDENCY_PROP_NAMES = [
  ...
  'additionalAppAnnotationProcessors',
  'additionalAndroidTestDependencies',
  'additionalClasspaths'
];
const DEPENDENCY_PREFIX = [
  'implementation',
  'annotationProcessor',
...
class ServerBuilder {
  ...
  async insertAdditionalDependencies () {
    ...
    const projectBuildPath = path.resolve(this.serverPath, 'build.gradle');
    const appBuildPath = path.resolve(this.serverPath, 'app', 'build.gradle');
    let projectConfiguration = await fs.readFile(projectBuildPath, 'utf8');
    let appConfiguration = await fs.readFile(appBuildPath, 'utf8');
    for (const i in DEPENDENCY_PROP_NAMES) {
      const propName = DEPENDENCY_PROP_NAMES[i];
      const prefix = DEPENDENCY_PREFIX[i];
      ...
      if (propName === 'additionalClasspaths') {
        projectConfiguration = updateDependencyLines(projectConfiguration, propName, deps);
      } else {
        appConfiguration = updateDependencyLines(appConfiguration, propName, deps);
      }
    }
    await fs.writeFile(appBuildPath, appConfiguration, 'utf8');
    await fs.writeFile(projectBuildPath, projectConfiguration, 'utf8');
  }
